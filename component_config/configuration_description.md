...
| `symbolic_period` | Optional symbolic-period **id** (e.g. `1` = Current Pay Period), a rolling window used instead of an explicit start/end date. Pick it with the Load Symbolic Periods dropdown (backed by the `list_symbolic_periods` sync action); WFM rejects a period name, only the id works. If set it takes precedence and Start/End Date are ignored — use one or the other, not both. Not supported for scheduling resources (they require an explicit Start/End window). |
| `primary_key` | Incremental load only. Column(s) that uniquely identify a row, used to upsert. Pick from the resource's output-table columns in Storage (a `list_columns` sync action, so it is populated only after the first run has created the table), or type the column name(s) directly when the picker can't resolve them (before the first run, on a dev branch, or with a custom output-bucket mapping). Overrides the resource registry default and enables incremental upsert even for a registry-keyless resource. Empty uses the resource default. |
